...
if($itemid) {
	...
	($item && $item['status'] > 2 && $item['items'] > 0 && $item['password'] == '') or wap_msg($L['msg_not_exist']);
	extract($item);
	$CAT = get_cat($catid);
	...
	$demo_url = 'index.php?moduleid='.$moduleid.'&amp;itemid='.$itemid;
	$next_photo = $page >= $items ? $demo_url : $demo_url.'&amp;page='.($page+1);
	$prev_photo = $page <= 1 ? $demo_url.'&amp;page='.$items : $demo_url.'&amp;page='.($page-1);
	$P = $T[$page-1];
